"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.CookiesMiddleware = void 0;
const common_1 = require("@nestjs/common");
const cookies_service_1 = require("./cookies.service");
const cookies_entity_1 = require("./cookies.entity");
let CookiesMiddleware = class CookiesMiddleware {
    constructor(cookiesService) {
        this.cookiesService = cookiesService;
    }
    async use(req, res, next) {
        const cookieConsent = this.cookiesService.getCookie(req, 'accept-cookies');
        if (cookieConsent) {
            this.cookiesService.setCookie(res, 'accept-cookies', 'true', {});
            const cookieLog = new cookies_entity_1.CookieEntity();
            cookieLog.ipAddress = req.ip;
            cookieLog.userAgent = req.get('User-Agent');
            cookieLog.acceptedAt = new Date();
            await this.cookiesService.saveCookie(cookieLog);
            res.locals.showCookieBanner = true;
        }
        else {
            res.locals.showCookieBanner = false;
        }
        next();
    }
};
exports.CookiesMiddleware = CookiesMiddleware;
exports.CookiesMiddleware = CookiesMiddleware = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [cookies_service_1.CookiesService])
], CookiesMiddleware);
//# sourceMappingURL=cookies.middleware.js.map